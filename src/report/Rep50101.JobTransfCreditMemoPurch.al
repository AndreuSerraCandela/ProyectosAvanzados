report 50101 "Job Transf Credit Memo Purch"
{
    Caption = 'Job Transfer to Credit Memo Purch';
    ProcessingOnly = true;

    dataset
    {
    }

    requestpage
    {

        layout
        {
            area(content)
            {
                group(Options)
                {
                    Caption = 'Options';
                    field(CreateNewCreditMemo; NewCreditMemo)
                    {
                        ApplicationArea = Jobs;
                        Caption = 'Create New Credit Memo';
                        ToolTip = 'Specifies if the batch job creates a new sales credit memo.';

                        trigger OnValidate()
                        begin
                            if NewCreditMemo then begin
                                CreditMemoNo := '';
                                if PostingDate = 0D then
                                    PostingDate := WorkDate;
                                CrMemoPostingDate := 0D;
                            end;
                        end;
                    }
                    field(PostingDate; PostingDate)
                    {
                        ApplicationArea = Jobs;
                        Caption = 'Posting Date';
                        ToolTip = 'Specifies the posting date for the document.';

                        trigger OnValidate()
                        begin
                            if PostingDate = 0D then
                                NewCreditMemo := false;
                        end;
                    }
                    field(AppendToCreditMemoNo; CreditMemoNo)
                    {
                        ApplicationArea = Jobs;
                        Caption = 'Append to Credit Memo No.';
                        ToolTip = 'Specifies the number of the credit memo that you want to append the lines to if you did not select the Create New Credit Memo field.';

                        trigger OnLookup(var Text: Text): Boolean
                        begin
                            Clear(PurchHeader);
                            PurchHeader.FilterGroup := 2;
                            PurchHeader.SetRange("Document Type", PurchHeader."Document Type"::"Credit Memo");
                            // PurchHeader.SetRange("Bill-to Customer No.", Job."Bill-to Customer No.");
                            PurchHeader.FilterGroup := 0;
                            if PAGE.RunModal(0, PurchHeader) = ACTION::LookupOK then
                                CreditMemoNo := PurchHeader."No.";
                            if CreditMemoNo <> '' then begin
                                PurchHeader.Get(PurchHeader."Document Type"::"Credit Memo", CreditMemoNo);
                                CrMemoPostingDate := PurchHeader."Posting Date";
                                NewCreditMemo := false;
                                PostingDate := 0D;
                            end;
                            if CreditMemoNo = '' then
                                InitReport;
                        end;

                        trigger OnValidate()
                        begin
                            if CreditMemoNo <> '' then begin
                                PurchHeader.Get(PurchHeader."Document Type"::"Credit Memo", CreditMemoNo);
                                CrMemoPostingDate := PurchHeader."Posting Date";
                                NewCreditMemo := false;
                                PostingDate := 0D;
                            end;
                            if CreditMemoNo = '' then
                                InitReport;
                        end;
                    }
                    field(CrMemoPostingDate; CrMemoPostingDate)
                    {
                        ApplicationArea = Jobs;
                        Caption = 'Cr. Memo Posting Date';
                        Editable = false;
                        ToolTip = 'Specifies the posting date of that credit memo if you filled the Append to Credit Memo No. field.';

                        trigger OnValidate()
                        begin
                            if PostingDate = 0D then
                                NewCreditMemo := false;
                        end;
                    }
                }
            }
        }

        actions
        {
        }

        trigger OnOpenPage()
        begin
            InitReport;
        end;
    }

    labels
    {
    }

    trigger OnInitReport()
    begin
        Done := false;
    end;

    trigger OnPostReport()
    begin
        Done := true;
    end;

    var
        Job: Record Job;
        PurchHeader: Record "Purchase Header";
        NewCreditMemo: Boolean;
        CreditMemoNo: Code[20];
        PostingDate: Date;
        CrMemoPostingDate: Date;
        Done: Boolean;

    procedure GetCreditMemoNo(var Done2: Boolean; var NewCreditMemo2: Boolean; var PostingDate2: Date; var CreditMemoNo2: Code[20])
    begin
        Done2 := Done;
        NewCreditMemo2 := NewCreditMemo;
        PostingDate2 := PostingDate;
        CreditMemoNo2 := CreditMemoNo;
    end;

    procedure InitReport()
    begin
        PostingDate := WorkDate;
        NewCreditMemo := true;
        CreditMemoNo := '';
        CrMemoPostingDate := 0D;
    end;

    procedure SetCustomer(JobNo: Code[20])
    begin
        Job.Get(JobNo);
    end;
}