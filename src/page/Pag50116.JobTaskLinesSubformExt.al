...
page 50116 "Job Task Lines Subform Ext"
{
        ...
        area(content)
        {
            repeater(Control1)
            {
                ...
                field("Amount Paid"; Rec."Amount Paid")// CalculaImportePagado())
                {
                    ...
                    begin
                        JobEntries.SetRange("Job No.", Rec."Job No.");
                        If Rec."Job Task Type" = Rec."Job Task Type"::Posting then
                            JobEntries.SetFilter("Job Task No.", Rec."Job Task No.")
                        else
                            JobEntries.SetFilter("Job Task No.", Rec.Totaling);
            ...
            group(Line)
            {
                ...
                group("&Job")
                {
                    ...
                    action("Actualizar Arbol")
                    {
                        ...
                        var
                            JobNo: Code[20];
                            CodProcesos: Codeunit ProcesosProyectos;
                        begin
                            if Rec."Job No." <> '' then
                                JobNo := Rec."Job No."
                            else
                            ...
                            CodProcesos.ActualizarArbolTareas(JobNo);
    ...
    local procedure CalculaNetoFactura(): Decimal
    var
        JobTask: Record "Job Task";
        NetoFactura: Decimal;
        Coste: Record "Job Ledger Entry";
    begin
        ...
        If JobTask.FindSet() then begin
            ...
            repeat
                ...
                if Coste.FindFirst() then
                    repeat
                        NetoFactura += Coste."Neto Factura";
                    until Coste.Next() = 0;
            until JobTask.Next() = 0;
        end;
        exit(NetoFactura);
