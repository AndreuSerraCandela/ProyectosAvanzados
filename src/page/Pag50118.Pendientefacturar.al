    ...
    actions
    {
        ...
        area(processing)
        {
            group("F&unctions")
            {
                ...
                //             end;
                //         }
                action("Pendiente facturar Mes actual")
                {
                    ApplicationArea = Jobs;
                    trigger OnAction()
                    var
                    ...
                    begin
                        ...
                        PagLineasPlanificacion.Caption := 'Pendientes factura Mes actual';
                        PagLineasPlanificacion.SetTableView(LineasPlanificacion);
                        PagLineasPlanificacion.RunModal();
                    end;

                }
                action("Pendiente facturar Mes anterior")
                {
                    ApplicationArea = Jobs;
                    trigger OnAction()
                    var
                    ...
                    begin
                        LineasPlanificacion.SetFilter("Line Type", '%1|%2', LineasPlanificacion."Line Type"::Billable, LineasPlanificacion."Line Type"::"Both Budget and Billable");
                        LineasPlanificacion.SetRange("Planning Date", CalcDate('<-1M-CM>', Today), CalcDate('<-1M+CM>', Today));
                        LineasPlanificacion.SetFilter("Qty. to Invoice", '<>%1', 0);
                        if LineasPlanificacion.FindFirst() then;
                        PagLineasPlanificacion.Caption := 'Pendientes factura Mes anterior';
                        PagLineasPlanificacion.SetTableView(LineasPlanificacion);
                        PagLineasPlanificacion.RunModal();
        ...
        area(Promoted)
        {
            group(Category_Process)
            {
                ...
                actionref(Pendiente_facturar_Mes_actual_promoted; "Pendiente facturar Mes actual") { }
                actionref(Pendiente_facturar_Mes_anterior_promoted; "Pendiente facturar Mes anterior") { }
